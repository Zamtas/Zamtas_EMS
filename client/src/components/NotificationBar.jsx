...
const NotificationBar = () => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data.data)) {
          ...
          setHasUnread(
            fetchedNotifications.some((notification) => !notification.read)
          );
        } else {
  ...
  const markAsRead = async (notificationId) => {
    try {
      ...
      setHasUnread(
        updatedNotifications.some((notification) => !notification.read)
      );
    } catch (error) {
  ...
  return (
    <>
      <div
        className={`fixed top-4 right-4 cursor-pointer text-2xl z-50 ${
