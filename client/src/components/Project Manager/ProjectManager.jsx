  ...
  const handleViewManager = async (manager) => {
    try {
      const { data } = await axios.get(
        Api.getManager.url.replace(":managerId", manager._id)
      );
  ...
  const handleSaveManager = async (managerId, updatedData) => {
    try {
      const response = await axios.put(
        Api.updateProjectManager.url.replace(":managerId", managerId),
        updatedData
      );
  ...
  return (
    <div className="p-6 bg-gray-50 rounded-lg shadow-md space-y-4">
      ...
      <ProjectManagerModal
        ...
        manager={currentManager || null} // Ensure null if undefined
        mode={modalMode}
      />
    </div>
  );
