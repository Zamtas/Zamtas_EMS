...
const ProductDetails = ({ projectId, setLastUpdatedBy }) => {
  ...
  const handleInputChange = (e, category, index) => {
    ...
    if (Array.isArray(updatedDetails[category])) {
      const updatedCategory = [...updatedDetails[category]];

      // If index is valid, update the value
      if (index >= 0 && index < updatedCategory.length) {
        updatedCategory[index] = e.target.value; // Use e.target.value here
      } else {
        console.error(
          `Index ${index} is out of bounds for category ${category}`
        );
      }
  ...
  const handleAdditionalMaterialChange = (index, field, value) => {
    ...
    updatedMaterial[index][field] = value; // Ensure correct value is set
    setAdditionalMaterial(updatedMaterial);
  };
  ...
  const calculateTotal = () => {
    return additionalMaterial.reduce(
              ...
              <tr key={idx}>
                <td className="p-2 border">
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleInputChange(e, "productDetails", idx)
                    }
                  />
                </td>
                <td className="p-2 border">
                ...
                <td className="p-2 border">
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleInputChange(e, "activationOption", idx)
                    }
                  />
                </td>
                <td className="p-2 border">
                ...
                <td className="p-2 border">
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleInputChange(e, "projectAddress", idx)
                    }
                  />
                </td>
                <td className="p-2 border">
              ...
              <tr key={idx}>
                ...
                <td className="p-2 border">
                  <AutoResizeTextarea
                    ...
                    onChange={(e) =>
                      handleInputChange(e, "sensorOtherDetails", idx)
                    }
                  />
                </td>
              </tr>
        ...
        <div className="w-1/2 pl-4">
          ...
          <div className="bg-white p-4 rounded shadow">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  <th className="border border-black bg-gray-100 p-2 text-left">
                    Material Required
                  </th>
                  <th className="border border-black bg-gray-100 p-2 text-left">
                    Value Pak Rupees
