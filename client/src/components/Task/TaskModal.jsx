...
const TaskModal = ({ isOpen, onClose, taskDetails, onAdd, isViewOnly }) => {
  ...
  const handleProjectChange = (e) => {
    ...
    axios
      .get(`${Api.getProjectDetail.url.replace(":projectId", projectId)}`)
      .then((res) => {
  ...
  const formattedEndTime = task.endTime
    ? moment(task.endTime, "HH:mm").format("h:mm A")
    : "";
  ...
  const handleAddCategory = () => {
    if (newCategory && !categories.includes(newCategory)) {
      axios
        .post(Api.addCategory.url, { category: newCategory })
        .then((res) => {
        ...
        {isViewOnly ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-8 rounded-xl shadow-2xl max-w-screen-lg mx-auto">
            ...
            <div className="space-y-6 bg-gradient-to-r from-blue-50 to-blue-100 p-6 rounded-lg shadow-lg">
              <h3 className="text-2xl font-semibold text-gray-800 border-b-2 border-blue-300 pb-3 mb-6">
                Task Information
              </h3>
              ...
              <div className="flex items-center text-lg">
                <span className="text-gray-700 font-bold flex-shrink-0 w-32">
                  Project:
                </span>
            ...
            <div className="space-y-6 bg-gradient-to-r from-green-50 to-green-100 p-6 rounded-lg shadow-lg">
              <h3 className="text-2xl font-semibold text-gray-800 border-b-2 border-green-300 pb-3 mb-6">
              ...
              <ul className="space-y-3 bg-white p-4 rounded-lg shadow-md">
                {task.assignedTo.map((user) => (
                  <li
                    key={user._id}
                    className="text-gray-700 font-medium flex items-center space-x-3 hover:bg-green-100 p-2 rounded-md transition duration-300 ease-in-out"
                  >
                    ...
                    styles={{
                      control: (provided, state) => ({
                        ...
                        boxShadow: state.isFocused
                          ...
                          : "none", // Blue ring on focus
                        padding: "0.5rem", // Equivalent to px-4 py-2
                        transition:
                          "box-shadow 150ms ease-in-out, border-color 150ms ease-in-out", // Smooth transition
                        "&:hover": {
