  ...
  const validateForm = () => {
    ...
    const errors = {
      clientName: !/^[A-Za-z\s]+$/.test(clientName)
        ? "Client Name should only contain alphabets and spaces."
        : "",
      clientContact: !/^[0-9]+$/.test(clientContact)
        ? "Client Contact should only contain numbers."
        : "",
      clientAddress: !/^[A-Za-z0-9\s.,-]+$/.test(clientAddress)
        ? "Client Address should only contain alphanumeric characters"
        : "",
      clientEmail: !/^\S+@\S+\.\S+$/.test(clientEmail)
        ? "Invalid email format."
        : "",
      clientContactPerson: !/^[A-Za-z\s]+$/.test(clientContactPerson)
        ? "Client Contact Person should only contain alphabets and spaces."
        : "",
    };
    ...
    try {
      ...
      if (modalType === "edit") {
        response = await axios.put(
          Api.updateClient.url.replace(":id", clientData._id),
          formData
        );
          ...
          <div className="grid grid-cols-2 gap-6">
            {Object.entries(formData).map(([key, value]) => (
              <div key={key} className="flex flex-col">
                <span className="text-sm font-medium text-gray-700 mb-1 capitalize">
                  {key.replace(/([A-Z])/g, " $1").trim()}:
                </span>
                <span className="text-lg text-gray-900">{value || "N/A"}</span>
              </div>
            ))}
          </div>
        ) : (
